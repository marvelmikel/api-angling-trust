        ...
        <div class="at-section">
            <div class="container">
                <div class="at-section__inner">
                    <h3 class="at-heading is-grass">{{ title }}</h3>
...
<script>
    export default {
        ...
        async created() {
            await this.waitForMember(async (member) => {
                switch (member.membership_type_slug) {
                    case 'trade-member':
                        this.title = 'Retail Associate Details';
                        break;
                    case 'trade-supporter':
                        this.title = 'Trade Supporter Details';
                        break;
                    default:
                        break;
                }

                const {data: { data: membershipTypeCategories}} = await this.$laravel_api.get(
                    `personal/membership_types/${member.membership_type_id}/categories`
                );

                this.options.category = membershipTypeCategories.map(membershipTypeCategory => ({
                    id: membershipTypeCategory.slug,
                    name: membershipTypeCategory.name
                }))
            });
        },
